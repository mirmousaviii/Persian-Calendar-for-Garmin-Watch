...
class PersianCalendarApp extends Application.AppBase {
    ...
    function getJalaliDateStr() {
        ...
        var monthStr = monthVal < 10 ? "0" + monthVal.toString() : monthVal.toString();
        var dayStr   = dayVal < 10   ? "0" + dayVal.toString()   : dayVal.toString();

        var jalaliStr = jalali.get("year").toString() + "/" + monthStr + "/" + dayStr;
        return jalaliStr;
    }
    ...
    function getGregorianDateStr() {
        var today = Gregorian.info(Time.now(), Time.FORMAT_SHORT);
        var monthStr = today.month < 10 ? "0" + today.month.toString() : today.month.toString();
        var dayStr   = today.day   < 10 ? "0" + today.day.toString()   : today.day.toString();
